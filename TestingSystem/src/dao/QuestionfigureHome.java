package dao;
import javax.persistence.EntityManager;
import javax.persistence.PersistenceContext;

import model.Questionfigure;
// default package
// Generated Mar 15, 2014 5:58:16 PM by Hibernate Tools 3.4.0.CR1
import javax.ejb.*;
import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

/**
 * Home object for domain model class Questionfigure.
 * @see .Questionfigure
 * @author Hibernate Tools
 */
@Stateless
public class QuestionfigureHome {

	private static final Log log = LogFactory.getLog(QuestionfigureHome.class);

	@PersistenceContext
	private EntityManager entityManager;

	public void persist(Questionfigure transientInstance) {
		log.debug("persisting Questionfigure instance");
		try {
			entityManager.persist(transientInstance);
			log.debug("persist successful");
		} catch (RuntimeException re) {
			log.error("persist failed", re);
			throw re;
		}
	}

	public void remove(Questionfigure persistentInstance) {
		log.debug("removing Questionfigure instance");
		try {
			entityManager.remove(persistentInstance);
			log.debug("remove successful");
		} catch (RuntimeException re) {
			log.error("remove failed", re);
			throw re;
		}
	}

	public Questionfigure merge(Questionfigure detachedInstance) {
		log.debug("merging Questionfigure instance");
		try {
			Questionfigure result = entityManager.merge(detachedInstance);
			log.debug("merge successful");
			return result;
		} catch (RuntimeException re) {
			log.error("merge failed", re);
			throw re;
		}
	}

	public Questionfigure findById(Integer id) {
		log.debug("getting Questionfigure instance with id: " + id);
		try {
			Questionfigure instance = entityManager.find(Questionfigure.class,
					id);
			log.debug("get successful");
			return instance;
		} catch (RuntimeException re) {
			log.error("get failed", re);
			throw re;
		}
	}
}
