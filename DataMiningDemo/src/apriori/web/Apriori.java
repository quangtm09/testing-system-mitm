 ...
 * rules with the given minimum confidence.<br/> The algorithm has an option to
 * mine class association rules. It is adapted as explained in the second
 * reference.<br/> <br/> For more information see:<br/> <br/> R. Agrawal, R.
 * Srikant: Fast Algorithms for Mining Association Rules in Large Databases. In:
 * 20th International Conference on Very Large Data Bases, 478-499, 1994.<br/>
 * <br/> Bing Liu, Wynne Hsu, Yiming Ma: Integrating Classification and
 * Association Rule Mining. In: Fourth International Conference on Knowledge
 * Discovery and Data Mining, 80-86, 1998.
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class Apriori
	...
	public void buildAssociations(Instances instances) throws Exception {
        ...
        final double lowerBoundMinSupportToUse =
                this.m_lowerBoundMinSupport * instances.numInstances() < 1.0
                ? 1.0 / instances.numInstances()
                : this.m_lowerBoundMinSupport;
        ...
        if (this.m_car && this.m_numRules == Integer.MAX_VALUE) {
            ...
            this.m_minSupport = this.m_minSupport < lowerBoundMinSupportToUse
      ...
      /*
            ...
            for (int i = 0; i < j + 1; i++) {
                supports[j - i] = (double) ((ItemSet) this.m_allTheRules[1].elementAt(j - i)).support() * -1;
            }
            indices = Utils.stableSort(supports);
            for (int i = 0; i < j + 1; i++) {
                sortedRuleSet[0].addElement(this.m_allTheRules[0].elementAt(indices[j - i]));
                sortedRuleSet[1].addElement(this.m_allTheRules[1].elementAt(indices[j - i]));
                sortedRuleSet[2].addElement(this.m_allTheRules[2].elementAt(indices[j - i]));
                if (this.m_metricType != CONFIDENCE || this.m_significanceLevel != -1) {
                    sortedRuleSet[3].addElement(this.m_allTheRules[3].elementAt(indices[j - i]));
                    sortedRuleSet[4].addElement(this.m_allTheRules[4].elementAt(indices[j - i]));
                    sortedRuleSet[5].addElement(this.m_allTheRules[5].elementAt(indices[j - i]));
                }
            }
            ...
            for (int i = sortedRuleSet[0].size() - 1;
                    i >= sortedRuleSet[0].size() - this.m_numRules && i >= 0; i--) {
            ...
            if (this.m_minSupport == lowerBoundMinSupportToUse || this.m_minSupport - this.m_delta > lowerBoundMinSupportToUse) {
                this.m_minSupport -= this.m_delta;
            } else {
                this.m_minSupport = lowerBoundMinSupportToUse;
            }

            necSupport = Math.round((float) (this.m_minSupport
    ...
    private void pruneRulesForUpperBoundSupport() {
        final int necMaxSupport = (int) (this.m_upperBoundMinSupport * this.m_instances.numInstances() + 0.5);
     ...
     * @param data the instances for which class association rules should be
     * mined
     * @throws Exception if rules can't be built successfully
     */
    @Override
	...
	public Enumeration listOptions() {
        ...
        final String string1 = "\tThe required number of rules. (default = " + this.m_numRules + ")",
                string2 =
                "\tThe minimum confidence of a rule. (default = " + this.m_minMetric + ")",
                string3 = "\tThe delta by which the minimum support is decreased in\n",
                string4 = "\teach iteration. (default = " + this.m_delta + ")",
                string5 =
        ...
        newVector.addElement(new Option("\tRemove columns that contain "
                + "all missing values (default = no)", "R", 0,
                "-R"));
	...
	public void setOptions(final String[] options) throws Exception {
        ...
        this.resetOptions();
        final String numRulesString = Utils.getOption('N', options),
        ...
        if (maxSupportString.length() != 0) {
            this.setUpperBoundMinSupport(new Double(maxSupportString).doubleValue());
        }
        if (minSupportString.length() != 0) {
            this.m_lowerBoundMinSupport = new Double(minSupportString).doubleValue();
        }
        if (significanceLevelString.length() != 0) {
            this.m_significanceLevel = new Double(significanceLevelString).doubleValue();
        }
	...
	public String toString() {
        ...
        if (this.m_Ls.size() <= 1) {
            return "<br>No large itemsets and rules found!<br>";
        }
        text.append("<br>Apriori<br>=======<br><br>");
        text.append("Minimum support: "
                + Utils.doubleToString(this.m_minSupport, 2)
        ...
        text.append(Utils.doubleToString(this.m_minMetric, 2) + "<br>");

        if (this.m_significanceLevel != -1) {
            text.append("Significance level: "
                    + Utils.doubleToString(this.m_significanceLevel, 2) + "<br>");
        }
        text.append("Number of cycles performed: " + this.m_cycles + "<br>");
        text.append("<br>Generated sets of large itemsets:<br>");
        if (!this.m_car) {
            for (int i = 0; i < this.m_Ls.size(); i++) {
                text.append("<br>Size of set of large itemsets L(" + (i + 1) + "): "
                        + ((FastVector) this.m_Ls.elementAt(i)).size() + "<br>");
                if (this.m_outputItemSets) {
                    text.append("<br>Large Itemsets L(" + (i + 1) + "):<br>");
                    for (int j = 0; j < ((FastVector) this.m_Ls.elementAt(i)).size(); j++) {
                        text.append(((AprioriItemSet) ((FastVector) this.m_Ls.elementAt(i)).elementAt(j)).toString(this.m_instances) + "<br>");
                    }
                }
            }
            text.append("<br>Best rules found:<br><br>");
            for (int i = 0; i < this.m_allTheRules[0].size(); i++) {
                text.append(Utils.doubleToString((double) i + 1,
                        (int) (Math.log(this.m_numRules) / Math.log(10) + 1), 0)
                        + ". " + ((AprioriItemSet) this.m_allTheRules[0].elementAt(i)).toString(this.m_instances)
                        + " ==> " + ((AprioriItemSet) this.m_allTheRules[1].elementAt(i)).toString(this.m_instances) + "    conf:("
                        + Utils.doubleToString(((Double) this.m_allTheRules[2].elementAt(i)).doubleValue(), 2) + ")");
                if (this.m_metricType != CONFIDENCE || this.m_significanceLevel != -1) {
                    text.append((this.m_metricType == LIFT ? " <" : "") + " lift:("
                            + Utils.doubleToString(((Double) this.m_allTheRules[3].elementAt(i)).doubleValue(), 2)
                            + ")" + (this.m_metricType == LIFT ? ">" : ""));
                    text.append((this.m_metricType == LEVERAGE ? " <" : "") + " lev:("
                            + Utils.doubleToString(((Double) this.m_allTheRules[4].elementAt(i)).doubleValue(), 2)
                            + ")");
                    text.append(" ["
                            + (int) (((Double) this.m_allTheRules[4].elementAt(i)).doubleValue() * this.m_instances.numInstances())
                            + "]" + (this.m_metricType == LEVERAGE ? ">" : ""));
                    text.append((this.m_metricType == CONVICTION ? " <" : "") + " conv:("
                            + Utils.doubleToString(((Double) this.m_allTheRules[5].elementAt(i)).doubleValue(), 2)
            ...
            for (int i = 0; i < this.m_Ls.size(); i++) {
                ...
                if (this.m_outputItemSets) {
                    text.append("<br>Large Itemsets L(" + (i + 1) + "):<br>");
                    for (int j = 0; j < ((FastVector) this.m_Ls.elementAt(i)).size(); j++) {
                        text.append(((ItemSet) ((FastVector) this.m_Ls.elementAt(i)).elementAt(j)).toString(this.m_instances) + "<br>");
                        text.append(((LabeledItemSet) ((FastVector) this.m_Ls.elementAt(i)).elementAt(j)).m_classLabel + "  ");
                        text.append(((LabeledItemSet) ((FastVector) this.m_Ls.elementAt(i)).elementAt(j)).support() + "<br>");
                    }
                }
            }
            text.append("<br>Best rules found:<br><br>");
            for (int i = 0; i < this.m_allTheRules[0].size(); i++) {
                text.append(Utils.doubleToString((double) i + 1,
                        (int) (Math.log(this.m_numRules) / Math.log(10) + 1), 0)
                        + ". " + ((ItemSet) this.m_allTheRules[0].elementAt(i)).toString(this.m_instances)
                        + " ==> " + ((ItemSet) this.m_allTheRules[1].elementAt(i)).toString(this.m_onlyClass) + "    conf:("
                        + Utils.doubleToString(((Double) this.m_allTheRules[2].elementAt(i)).doubleValue(), 2) + ")");
    ...
    public void addRulesToList() {
        this.ruleList = new ArrayList<Rule>();
        if (this.m_Ls.size() < 1) {
            return;
        }
        for (int i = 0; i < this.m_allTheRules[0].size(); i++) {
            final String antecedentFull = ((AprioriItemSet) this.m_allTheRules[0].elementAt(i)).toString(this.m_instances);
            final String consequentFull = ((AprioriItemSet) this.m_allTheRules[1].elementAt(i)).toString(this.m_instances);

            final String antecedentSupport = antecedentFull.substring(antecedentFull.lastIndexOf(":") + 2, antecedentFull.length());
            final String consequentSupport = consequentFull.substring(consequentFull.lastIndexOf(":") + 2, consequentFull.length());

            final String antecedent = antecedentFull.substring(0, antecedentFull.lastIndexOf("Support") - 1);
            final String consequent = consequentFull.substring(0, consequentFull.lastIndexOf("Support") - 1);

            final String confidence = Utils.doubleToString(((Double) this.m_allTheRules[2].elementAt(i)).doubleValue(), 2);

            final Rule rule = new Rule();

            rule.setAntecedent(antecedent);
            rule.setConsequent(consequent);
            rule.setAntecedentSupport(Double.parseDouble(antecedentSupport));
            rule.setConsequentSupport(Double.parseDouble(consequentSupport));
            rule.setConfidence(Double.parseDouble(confidence));



            if (this.m_metricType != CONFIDENCE || this.m_significanceLevel != -1) {
                rule.setLift(Double.parseDouble(Utils.doubleToString(((Double) this.m_allTheRules[3].elementAt(i)).doubleValue(), 2)));
                rule.setLeverage(Double.parseDouble(Utils.doubleToString(((Double) this.m_allTheRules[4].elementAt(i)).doubleValue(), 2)));
                rule.setLeverageExamples((int) (((Double) this.m_allTheRules[4].elementAt(i)).doubleValue() * this.m_instances.numInstances()));
                rule.setConviction(Double.parseDouble(Utils.doubleToString(((Double) this.m_allTheRules[5].elementAt(i)).doubleValue(), 2)));
            } else {
                rule.setLift(0);
                rule.setLeverage(0);
                rule.setLeverageExamples(0);
                rule.setConviction(0);
            }
...
//        text.append(dataManager.displaySuccessBlock("Generated " + m_Ls.size() + " sets of large itemsets:"));
//        for (int i = 0; i < m_Ls.size(); i++) {
//            text.append(dataManager.displaySuccessBlock("Large Itemsets L(" + (i + 1) + ") includes " + ((FastVector) m_Ls.elementAt(i)).size()
//                    + " itemsets: <a id=\"aprioriLinkItemsets_" + (i + 1) + "\" href=\"javascript:;\" onclick=\"$('div#aprioriItemsets_" + (i + 1) + "').toggle();"
//                    + "if($('#aprioriLinkItemsets_" + (i + 1) + "').text() == 'Show'){"
//                    + "$('#aprioriLinkItemsets_" + (i + 1) + "').text('Hide');"
//                    + "} else {"
//                    + "$('#aprioriLinkItemsets_" + (i + 1) + "').text('Show');"
//                    + "}\">Show</a>"));
//            text.append("<div id=\"aprioriItemsets_" + (i + 1) + "\" style=\"display: none\">");
//            for (int j = 0; j < ((FastVector) m_Ls.elementAt(i)).size(); j++) {
//                text.append(((AprioriItemSet) ((FastVector) m_Ls.elementAt(i)).elementAt(j)).toString(m_instances) + "<br>");
//            }
//            text.append("</div>");
//        }
        String styleWidthTable = "style=\"width: 1000px;\"";
        if (this.m_metricType != CONFIDENCE || this.m_significanceLevel != -1) {
            styleWidthTable = "style=\"width: 1100px;\"";
        }

        text.append("<h3>Best association rules found:</h3>");
        text.append("<table id=\"newspaper-a\"" + styleWidthTable + ">");
        text.append("<tr><th>Number</th>");
        text.append("<th scope=\"col\">Rule</th>");
        text.append("<th>Confidence</th>");
        text.append("<th scope=\"col\">Antecedent Support</th>");
        text.append("<th scope=\"col\">Consequent Support</th>");
        if (this.m_metricType != CONFIDENCE || this.m_significanceLevel != -1) {
            text.append("<th>Lift</th>");
            text.append("<th>Leverage</th>");
            text.append("<th>Conviction</th></tr>");
        }
        for (int i = 0; i < this.m_allTheRules[0].size(); i++) {
            final String antecedentFull = ((AprioriItemSet) this.m_allTheRules[0].elementAt(i)).toString(this.m_instances);
            final String consequentFull = ((AprioriItemSet) this.m_allTheRules[1].elementAt(i)).toString(this.m_instances);
            ...
            text.append("<tr>");
            text.append("<td>" + Utils.doubleToString((double) i + 1,
                    (int) (Math.log(this.m_numRules) / Math.log(10) + 1), 0)
                    + "</td>" + "<td><span style=\"color: black\"><strong>IF</strong></span><br><span style=\"color: #347235\">" + antecedent
                    + "</span><br><span style=\"color: black\"><strong>THEN</strong></span><br><span style=\"color: #347235\">" + consequent + "</span></td>"
                    + "<td>" + confidence + "</td>"
                    + "<td>" + antecedentSupport + "</td>"
                    + "<td>" + consequentSupport + "</td>");

            if (this.m_metricType != CONFIDENCE || this.m_significanceLevel != -1) {
                text.append("<td>" + Utils.doubleToString(((Double) this.m_allTheRules[3].elementAt(i)).doubleValue(), 2) + "</td>");
                text.append("<td>" + Utils.doubleToString(((Double) this.m_allTheRules[4].elementAt(i)).doubleValue(), 2));
                text.append("[" + (int) (((Double) this.m_allTheRules[4].elementAt(i)).doubleValue() * this.m_instances.numInstances()) + "]</td>");
                text.append("<td>" + Utils.doubleToString(((Double) this.m_allTheRules[5].elementAt(i)).doubleValue(), 2) + "</td>");
            }
            text.append("</tr>");
        }
        text.append("</table>");
    ...
    private void findLargeItemSets() throws Exception {
        ...
        necMaxSupport = (int) (this.m_upperBoundMinSupport * this.m_instances.numInstances() + 0.5);
    ...
    private void findRulesBruteForce() throws Exception {
        ...
        for (int j = 1; j < this.m_Ls.size(); j++) {
            ...
            while (enumItemSets.hasMoreElements()) {
                final AprioriItemSet currentItemSet = (AprioriItemSet) enumItemSets.nextElement();
                //AprioriItemSet currentItemSet = new AprioriItemSet((ItemSet)enumItemSets.nextElement());
                rules = currentItemSet.generateRulesBruteForce(this.m_minMetric, this.m_metricType,
                        this.m_hashtables, j + 1,
                        this.m_instances.numInstances(),
                        this.m_significanceLevel);
    ...
    private void findRulesQuickly() throws Exception {
        ...
        for (int j = 1; j < this.m_Ls.size(); j++) {
            ...
            while (enumItemSets.hasMoreElements()) {
                final AprioriItemSet currentItemSet = (AprioriItemSet) enumItemSets.nextElement();
                //AprioriItemSet currentItemSet = new AprioriItemSet((ItemSet)enumItemSets.nextElement());
                rules = currentItemSet.generateRules(this.m_minMetric, this.m_hashtables, j + 1);
                for (int k = 0; k < rules[0].size(); k++) {
                    this.m_allTheRules[0].addElement(rules[0].elementAt(k));
                    this.m_allTheRules[1].addElement(rules[1].elementAt(k));
                    this.m_allTheRules[2].addElement(rules[2].elementAt(k));
    ...
    private void findLargeCarItemSets() throws Exception {
        ...
        final double nextMaxSupport = this.m_upperBoundMinSupport * this.m_instances.numInstances();
    ...
    private void findCarRulesQuickly() throws Exception {
        ...
        for (int j = 0; j < this.m_Ls.size(); j++) {
            ...
            while (enumLabeledItemSets.hasMoreElements()) {
                final LabeledItemSet currentLabeledItemSet = (LabeledItemSet) enumLabeledItemSets.nextElement();
	...
	public String getRevision() {
        return RevisionUtils.extract("$Revision: 7477 $");
    }
      ...
      if (!this.m_car) {
        for (int i = 0; i < this.m_Ls.size(); i++) {
          ...
          if (this.m_outputItemSets) {
            ...
            for (int j = 0; j < ((FastVector) this.m_Ls.elementAt(i)).size(); j++) {
				text.append(((AprioriItemSet) ((FastVector) this.m_Ls.elementAt(i))
        ...
        for (int i = 0; i < this.m_allTheRules[0].size(); i++) {
          text.append(Utils.doubleToString((double) i + 1,
              ...
              + Utils.doubleToString(
                  ((Double) this.m_allTheRules[2].elementAt(i)).doubleValue(), 2)
              + ")");
          if (this.m_metricType != CONFIDENCE || this.m_significanceLevel != -1) {
            text.append((this.m_metricType == LIFT ? " <" : "")
                + " lift:("
                + Utils.doubleToString(
                    ((Double) this.m_allTheRules[3].elementAt(i)).doubleValue(), 2)
                + ")" + (this.m_metricType == LIFT ? ">" : ""));
            text.append((this.m_metricType == LEVERAGE ? " <" : "")
                + " lev:("
                + Utils.doubleToString(
                    ((Double) this.m_allTheRules[4].elementAt(i)).doubleValue(), 2)
                + ")");
            text.append(" ["
                + (int) (((Double) this.m_allTheRules[4].elementAt(i)).doubleValue() * this.m_instances
                    .numInstances()) + "]"
                + (this.m_metricType == LEVERAGE ? ">" : ""));
            text.append((this.m_metricType == CONVICTION ? " <" : "")
                + " conv:("
                + Utils.doubleToString(
                    ((Double) this.m_allTheRules[5].elementAt(i)).doubleValue(), 2)
                + ")" + (this.m_metricType == CONVICTION ? ">" : ""));
          }
          text.append('\n');
        }
          ...
          if (this.m_outputItemSets) {
            ...
            for (int j = 0; j < ((FastVector) this.m_Ls.elementAt(i)).size(); j++) {
              text.append(((ItemSet) ((FastVector) this.m_Ls.elementAt(i))
                  .elementAt(j)).toString(this.m_instances) + "\n");
              text.append(((LabeledItemSet) ((FastVector) this.m_Ls.elementAt(i))
        ...
        for (int i = 0; i < this.m_allTheRules[0].size(); i++) {
          text.append(Utils.doubleToString((double) i + 1,
              ...
              + ((ItemSet) this.m_allTheRules[0].elementAt(i)).toString(this.m_instances)
              + " ==> "
              + ((ItemSet) this.m_allTheRules[1].elementAt(i)).toString(this.m_onlyClass)
              + "    conf:("
              + Utils.doubleToString(
                  ((Double) this.m_allTheRules[2].elementAt(i)).doubleValue(), 2)
              + ")");
