...
public class LabeledItemSet extends ItemSet implements Serializable,
	 ...
	 *            the class lebel
	 */
	public LabeledItemSet(final int totalTrans, final int classLabel) {
	 ...
	 */
	@Override
	public final boolean equals(final Object itemSet) {

		if (!this.equalCondset(itemSet)) {
			return false;
		}
		if (this.m_classLabel != ((LabeledItemSet) itemSet).m_classLabel) {
			return false;
		}
	 ...
	 */
	public final boolean equalCondset(final Object itemSet) {
		...
		for (int i = 0; i < this.m_items.length; i++) {
			if (this.m_items[i] != ((ItemSet) itemSet).itemAt(i)) {
				return false;
			}
		}
		return true;
	...
	public static FastVector mergeAllItemSets(final FastVector itemSets, final int size,
		...
		for (int i = 0; i < itemSets.size(); i++) {
			final LabeledItemSet first = (LabeledItemSet) itemSets.elementAt(i);
	 ...
	 *            the instances
	 * @param invert
	 *            flag; if true only the class attribute remains, otherweise the
	 *            class attribute is the only attribute that is deleted.
	 * @throws Exception
	 *             exception if instances cannot be splitted
	 * @return Instances without the class attribute or instances with only the
	 *         class attribute
	 */
	public static Instances divide(final Instances instances, final boolean invert)
			throws Exception {

		final Instances newInstances = new Instances(instances);
	 ...
	 * sets (singletons). The ordering of values in the header file determines
	 * the lexicographic order. Each item set knows its class label.
	 *
	 * @return a set of item sets, each containing a single item
	 * @param instancesNoClass
	 *            instances without the class attribute
	 * @param classes
	 *            the values of the class attribute sorted according to
	 *            instances
	 * @exception Exception
	 *                if singletons can't be generated successfully
	 */
	public static FastVector singletons(final Instances instancesNoClass,
			final Instances classes) throws Exception {

		final FastVector cSet, setOfItemSets = new FastVector();
		...
		for (int i = 0; i < instancesNoClass.numAttributes(); i++) {
			...
			for (int j = 0; j < instancesNoClass.attribute(i).numValues(); j++) {
				for (int k = 0; k < classes.attribute(0).numValues(); k++) {
					current = new LabeledItemSet(
							instancesNoClass.numInstances(), k);
	 ...
	 *
	 * @param toPrune
	 *            the set of (k)-item sets to be pruned
	 * @param kMinusOne
	 *            the (k-1)-item sets to be used for pruning
	 * @return the pruned set of item sets
	 */
	public static FastVector pruneItemSets(final FastVector toPrune,
			final Hashtable kMinusOne) {

		final FastVector newVector = new FastVector(toPrune.size());
		int help, j;

		for (int i = 0; i < toPrune.size(); i++) {
			final LabeledItemSet current = (LabeledItemSet) toPrune.elementAt(i);
			...
			for (j = 0; j < current.m_items.length; j++) {
				if (current.m_items[j] != -1) {
					...
					if (kMinusOne.get(current) != null
							&& current.m_classLabel == ((Integer) kMinusOne
	 ...
	 *
	 * @param instanceNoClass
	 *            instances without the class attribute
	 * @param instanceClass
	 *            the values of the class attribute sorted according to
	 *            instances
	 */
	public final void upDateCounter(final Instance instanceNoClass,
			final Instance instanceClass) {
	 ...
	 *            instances
	 */
	public static void upDateCounters(final FastVector itemSets,
			final Instances instancesNoClass, final Instances instancesClass) {
	 ...
	 * @param noPrune
	 *            flag indicating whether the rules are pruned accoridng to the
	 *            minimum confidence value
	 * @return a set of rules
	 */
	public final FastVector[] generateRules(final double minConfidence,
			final boolean noPrune) {

		final FastVector premises = new FastVector(), consequences = new FastVector(), conf = new FastVector();
		...
		consequence = new ItemSet(this.m_totalTransactions);
		final int[] premiseItems = new int[this.m_items.length];
		final int[] consequenceItems = new int[1];
