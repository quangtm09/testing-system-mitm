...
public class AssociatorEvaluation implements RevisionHandler {
	 ...
	 *
	 * @param associator
	 *            the associator to generate the string for
	 * @return the option string
	 */
	protected static String makeOptionString(final Associator associator) {
		StringBuffer text;
		...
		if (associator instanceof OptionHandler) {
			...
			final Enumeration enm = ((OptionHandler) associator).listOptions();
			while (enm.hasMoreElements()) {
				final Option option = (Option) enm.nextElement();
	 ...
	 *
	 * @param associatorString
	 *            class of associator as a string
	 * @param options
	 ...
	 */
	public static String evaluate(final String associatorString, final String[] options)
			throws Exception {
		Associator associator;

		// Create associator
		try {
			associator = (Associator) Class.forName(associatorString)
					.newInstance();
	 ...
	 *             if evaluation fails
	 */
	public static String evaluate(final Associator associator, final String[] options)
			throws Exception {
		...
		final String results = eval.evaluate(associator,
	 ...
	 *            the Associator to evaluate
	 * @param data
	 *            the data to run the associator with
	 * @return the generated output string
	 * @throws Exception
	 *             if evaluation fails
	 */
	public String evaluate(final Associator associator, final Instances data)
			throws Exception {
		...
		this.m_Result.append("Elapsed time: "
	...
	public boolean equals(final Object obj) {
		...
		final AssociatorEvaluation cmp = (AssociatorEvaluation) obj;

		// TODO: better comparison???
		final String associatingResults1 = this.m_Result.toString().replaceAll(
				"Elapsed time.*", "");
		final String associatingResults2 = cmp.m_Result.toString().replaceAll(
				"Elapsed time.*", "");
	 ...
	 *
	 * @param title
	 *            the title to print before the result
	 * @return the summary string
	 */
	public String toSummaryString(final String title) {
		StringBuffer result;
	...
	public static void main(final String[] args) {
		try {
			...
			final String associator = args[0];
			args[0] = "";
