...
public class AprioriItemSet extends ItemSet implements Serializable,
	 ...
	 *            how many times the consequence occurs independent of the
	 *            premise
	 * @return the lift on the training data
	 */
	public double liftForRule(final AprioriItemSet premise,
			final AprioriItemSet consequence, final int consequenceCount) {
	...
	public double leverageForRule(final AprioriItemSet premise,
			final AprioriItemSet consequence, final int premiseCount, final int consequenceCount) {
		final double coverageForItemSet = (double) consequence.m_counter
				/ (double) this.m_totalTransactions;
		final double expectedCoverageIfIndependent = (double) premiseCount / (double) this.m_totalTransactions
				* ((double) consequenceCount / (double) this.m_totalTransactions);
	...
	public double convictionForRule(final AprioriItemSet premise,
			final AprioriItemSet consequence, final int premiseCount, final int consequenceCount) {
		final double num = (double) premiseCount
				* (double) (this.m_totalTransactions - consequenceCount)
				/ this.m_totalTransactions;
		final double denom = premiseCount - consequence.m_counter + 1;

		if (num < 0 || denom < 0) {
			System.err.println("*** " + num + " " + denom);
			System.err.println("premis count: " + premiseCount
					+ " consequence count " + consequenceCount
					+ " total trans " + this.m_totalTransactions);
		}
		return num / denom;
	}
	 ...
	 * @param hashtables
	 *            containing all(!) previously generated item sets
	 * @param numItemsInSet
	...
	public FastVector[] generateRules(final double minConfidence,
			final FastVector hashtables, final int numItemsInSet) {
		...
		final Hashtable hashtable = (Hashtable) hashtables
	 ...
	 *
	 * @param minMetric
	 *            the minimum metric (confidence, lift, leverage, improvement)
	 *            the rules have to have
	 * @param metricType
	...
	public final FastVector[] generateRulesBruteForce(final double minMetric,
			final int metricType, final FastVector hashtables, final int numItemsInSet,
			final int numTransactions, final double significanceLevel) throws Exception {

		final FastVector premises = new FastVector(), consequences = new FastVector(), conf = new FastVector(), lift = new FastVector(), lev = new FastVector(), conv = new FastVector();
		...
		for (int j = 1; j < max; j++) {
			...
			if (numItemsInPremise < numItemsInSet) {
				...
				if (significanceLevel != -1) {
					...
					contingencyTable[1][1] = numTransactions
							- premise.m_counter
							- consequenceUnconditionedCounter + consequence.m_counter;
					chiSquared = ContingencyTables.chiSquared(contingencyTable,
							false);
				}
				...
				if (metricType == 0) {
					...
					if (!(metric < minMetric)
						...
						lift.addElement(new Double(this.liftForRule(premise,
								consequence, consequenceUnconditionedCounter)));
						lev.addElement(new Double(this.leverageForRule(premise,
								consequence, premise.m_counter,
								consequenceUnconditionedCounter)));
						conv.addElement(new Double(this.convictionForRule(premise,
								consequence, premise.m_counter,
								consequenceUnconditionedCounter)));
					}
				} else {
	 ...
	 *
	 * @param rules
	 *            all the rules having (k-1)-item sets as consequences
	 * @param numItemsInSet
	 *            the size of the item set for which the rules are to be
	 *            generated
	 * @param numItemsInConsequence
	 *            the value of (k-1)
	 * @param minConfidence
	 *            the minimum confidence a rule has to have
	 * @param hashtables
	 *            the hashtables containing all(!) previously generated item
	 *            sets
	 * @return all the rules having (k)-item sets as consequences
	 */
	private final FastVector[] moreComplexRules(final FastVector[] rules,
			final int numItemsInSet, final int numItemsInConsequence, final double minConfidence,
		...
		if (numItemsInSet > numItemsInConsequence + 1) {
			...
			newConsequences = mergeAllItemSets(rules[1],
					numItemsInConsequence - 1, this.m_totalTransactions);
			...
			moreResults = this.moreComplexRules(result, numItemsInSet,
