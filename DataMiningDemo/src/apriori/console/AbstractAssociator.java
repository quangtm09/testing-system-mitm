 ...
 */
public abstract class AbstractAssociator implements Cloneable, Associator,
	 ...
	 * (optional) arguments to pass to it's setOptions method. If the associator
	 * implements OptionHandler and the options parameter is non-null, the
	 * associator will have it's options set.
	 *
	 * @param associatorName
	 *            the fully qualified class name of the associator
	 * @param options
	 *            an array of options suitable for passing to setOptions. May be
	 *            null.
	 * @return the newly created associator, ready for use.
	 * @exception Exception
	 *                if the associator name is invalid, or the options supplied
	 *                are not acceptable to the associator
	 */
	public static Associator forName(final String associatorName, final String[] options)
			throws Exception {
	 ...
	 *
	 * @param model
	 *            the associator to copy
	 * @return a deep copy of the associator
	 * @exception Exception
	 *                if an error occurs
	 */
	public static Associator makeCopy(final Associator model) throws Exception {
	 ...
	 *                if an error occurs
	 */
	public static Associator[] makeCopies(final Associator model, final int num)
			throws Exception {

		if (model == null) {
			throw new Exception("No model associator set");
		}
		final Associator[] associators = new Associator[num];
		final SerializedObject so = new SerializedObject(model);
	 ...
	 *
	 * @param associator
	 *            the associator to run
	 * @param options
	 *            the commandline options
	 */
	protected static void runAssociator(final Associator associator, final String[] options) {
		try {
			System.out.println(AssociatorEvaluation.evaluate(associator,
					options));
