...
/**
 * The listener interface for receiving intervewScheduleEvent events.
 * The class that is interested in processing a intervewScheduleEvent
 * event implements this interface, and the object created
 * with that class is registered with a component using the
 * component's <code>addIntervewScheduleEventListener<code> method. When
 * the intervewScheduleEvent event occurs, that object's appropriate
 * method is invoked.
 *
 * @see IntervewScheduleEventEvent
 */
@Name("interviewScheduleListener")
public class IntervewScheduleEventListener {
	...
	public void handleInterviewScheduleModified(InterviewSchedule old,
		...
		if (!old.getItsTechnicalInterviewTime().equals(newIS.getItsTechnicalInterviewTime())) {
			MailEvent meetingRequest = buildMailEvent(newIS, newInterviewers,
					Method.REQUEST, "");
			meetingRequest.setUid(new Uid(old.getIcalendarUID()));
			try {
				meetingRequest.sendAppointment(MailSession.instance());
			} catch (Exception e) {
				// TODO Auto-generated catch block
